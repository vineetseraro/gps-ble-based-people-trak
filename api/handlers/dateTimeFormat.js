...
const dateTimeFormathelper = require('../helpers/dateTimeFormat');
...
module.exports.getDateTimeFormats = (event, context, callback) => {
  ...
  commonHelper.decryptDbURI().then(dbURI => {
    ...
    bluebirdPromise
      .all([dateTimeFormathelper.getdateTimeFormats(), dateTimeFormathelper.countdateTimeFormats()])
      .then(resultObj => {
        const response = akResponse.listSuccess(resultObj, 'Datetime Formats List');
