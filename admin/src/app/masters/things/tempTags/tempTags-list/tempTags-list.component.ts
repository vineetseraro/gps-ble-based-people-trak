...
@Component({
  selector: 'app-tempTags-list',
  templateUrl: './tempTags-list.component.html',
  providers: [GlobalService]
})
export class TempTagsListComponent implements OnInit {
  @ViewChild('dt') public dataTable: DataTable;

  previousQuery: string;
  loader = false;
  tempTagsList: TempTags[];
  totalRecords = 0;
  pagingmessage = '';
  startPageIndex = 1;
  endPageIndex = 10;
  ...
  */
  ...
  public getCollectionList(query: string) {
    this.loader = true;
    this.thingsService.getAllTempTags(query).subscribe(
      (data: any) => {
        console.log(data);
        this.tempTagsList = data.data;
        if (this.temperatureUnit === 'fahrenheit') {
          this.tempTagsList.map(dt => {
        ...
        this.setPageinationgMessage(this.tempTagsList.length);
        this.loader = false;
