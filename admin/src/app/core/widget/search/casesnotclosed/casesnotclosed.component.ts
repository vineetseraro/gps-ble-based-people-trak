...
@Component({
  selector: 'app-search-casesnotclosed',
  templateUrl: './casesnotclosed.component.html',
  styleUrls: ['./casesnotclosed.component.css']
})
export class CasesnotclosedComponent implements OnInit {

  searchForm: FormGroup; // our model driven form
  orderStatus = this.getOrderStatusDropDown();
  dropDown: Dropdown[];
  ...
  searchRecord(searchValue:any) {
    ...
    if (searchValue.timeelapsed !== null && searchValue.timeelapsed !== "")
      query += "&timeElapsed=" + searchValue.timeelapsed;
