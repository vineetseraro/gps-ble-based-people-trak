...
@Component({
  selector: 'app-search-sensorlocator',
  templateUrl: './sensorlocator.component.html',
  /*styleUrls: ['./diagn_sensortracking.component.css'],*/
...
closeDialog() {
  ...
  searchRecord(searchValue:any = null) {
    ...
    if (this.showmessage === false )  {
      ...
      if (searchValue.sensor !== null && searchValue.sensor !== '') {
        this.searchObj.sensor = { value: searchValue.sensor, matchMode: 'Contains' }
      }
      if (searchValue.trackedFrom !== null && searchValue.trackedFrom !== '') {
        this.searchObj.trackedFrom = { value: this.globalService.formatDate(searchValue.trackedFrom + ':00'), matchMode: 'Contains' }
      }
      if (searchValue.trackedTo !== null && searchValue.trackedTo !== '') {
        this.searchObj.trackedTo = { value: this.globalService.formatDate(searchValue.trackedTo + ':00'), matchMode: 'Contains' }
      }
      this.commonService.notifyOther({option: 'sensorlocator_search', value: this.searchObj});
      this.closeSearchBox('');
    }
  }
