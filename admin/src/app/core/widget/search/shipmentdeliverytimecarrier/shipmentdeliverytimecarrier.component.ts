...
@Component({
  selector: 'app-search-shipmentdeliverytimecarrier',
  templateUrl: './shipmentdeliverytimecarrier.component.html',
  styleUrls: ['./shipmentdeliverytimecarrier.component.css']
})
export class ShipmentdeliverytimecarrierComponent implements OnInit {
  ...
  searchRecord(searchValue:any) {
    ...
    if (searchValue.scheduledFrom !== null && searchValue.scheduledFrom !== '')
      query += '&etdFrom=' + this.globalService.formatDate(searchValue.scheduledFrom);
    ...
    if (searchValue.scheduledTo !== null && searchValue.scheduledTo !== '')
      query += '&etdTo=' + this.globalService.formatDate(searchValue.scheduledTo);
    this.commonService.notifyOther({ option: 'shipmentdeliverytimecarrier_search', value: query });
    this.closeit('');
  }
  ...
  clickout(event:any) {
    //this.id = document.getElementById("cars");
    if (
