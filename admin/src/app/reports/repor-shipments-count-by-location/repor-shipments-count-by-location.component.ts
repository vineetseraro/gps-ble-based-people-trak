...
@Component({
  selector: 'app-repor-shipments-count-by-location',
  templateUrl: './repor-shipments-count-by-location.component.html',
  styleUrls: ['./repor-shipments-count-by-location.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReporShipmentsCountByLocationComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'shipmentscountbylocation_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('internal-external-shipments', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'internalExternalShipment';
