...
@Component({
  selector: 'app-report-stationary-shipments',
  templateUrl: './report-stationary-shipments.component.html',
  styleUrls: ['./report-stationary-shipments.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportStationaryShipmentsComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'stationaryshipment_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('stationary-shipments', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'stationaryShipments';
