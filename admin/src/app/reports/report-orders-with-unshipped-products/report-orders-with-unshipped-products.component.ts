...
@Component({
  selector: 'app-report-orders-with-unshipped-products',
  templateUrl: './report-orders-with-unshipped-products.component.html',
  styleUrls: ['./report-orders-with-unshipped-products.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportOrdersWithUnshippedProductsComponent implements OnInit {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('orders-with-unshipped-products', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'ordersWithUnshippedProducts';
