...
@Component({
  selector: 'app-report-undelivered-products',
  templateUrl: './report-undelivered-products.component.html',
  styleUrls: ['./report-undelivered-products.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportUndeliveredProductsComponent implements OnInit {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('undelivered-products', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'undeliveredProducts';
