...
@Component({
  selector: 'app-report-sku-sensor-mapping',
  templateUrl: './report-sku-sensor-mapping.component.html',
  styleUrls: ['./report-sku-sensor-mapping.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportSkuSensorMappingComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'skusensor') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('product-thing-mapping', query).subscribe((data:any) => {
  ...
  openReport(id:any) {
    this.router.navigate(['/reports/productsensorhistory/' + id]);
  }
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'productThingMapping';
