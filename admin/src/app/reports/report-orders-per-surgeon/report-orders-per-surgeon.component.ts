...
@Component({
  selector: 'app-report-orders-per-surgeon',
  templateUrl: './report-orders-per-surgeon.component.html',
  styleUrls: ['./report-orders-per-surgeon.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportOrdersPerSurgeonComponent implements OnInit {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('orders-per-surgeon', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'ordersBySurgery';
