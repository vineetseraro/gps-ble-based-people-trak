...
@Component({
  selector: 'app-report-casesperhospital',
  templateUrl: './report-casesperhospital.component.html',
  styleUrls: ['./report-casesperhospital.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportCasesperhospitalComponent implements OnInit {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('orders-per-hospital', query).subscribe((data:any) => {
      console.log("I am calling search");
   ...
   * @memberof ReportCasesperhospitalComponent
   */
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'ordersPerHospital';
