...
@Component({
  selector: 'app-report-shipments-hard-delivered-by-web-admin',
  templateUrl: './report-shipments-hard-delivered-by-web-admin.component.html',
  styleUrls: ['./report-shipments-hard-delivered-by-web-admin.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportShipmentsHardDeliveredByWebAdminComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'shipmentharddelivered_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('shipment-hard-delivered', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'shipmentsHardDelivered';
