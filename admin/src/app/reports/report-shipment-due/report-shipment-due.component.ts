...
@Component({
  selector: 'app-report-shipment-due',
  templateUrl: './report-shipment-due.component.html',
  styleUrls: ['./report-shipment-due.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportShipmentDueComponent implements OnInit {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('shipment-due', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'shipmentDue';
