...
@Component({
  selector: 'app-report-casespercity',
  templateUrl: './report-casespercity.component.html',
  styleUrls: ['./report-casespercity.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportCasespercityComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'casespercity_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('orders-by-city', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'ordersPerCity';
