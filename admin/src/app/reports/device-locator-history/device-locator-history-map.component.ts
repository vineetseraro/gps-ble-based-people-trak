...
//
@Component({
    selector: 'app-devicelocatorhistory-map',
    templateUrl: './device-locator-history-map.component.html',
    providers: [GlobalService, ReportService, LocationService,
        LocatormapService, LocatortypeFactory, RoutemapService, ThingsService],
})
export class DeviceLocatorHistoryMapComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.thingsService.getApp(this.deviceId).subscribe((data: any) => {
            ...
            this.reportService.deviceLocatorHistoryMap(
                this.deviceId,
    ...
    initializeMap(pastLocations: any, enableLiveTracking: any) {
        ...
        this.map = L.map('map').on('load', function () {
            ...
            if (enableLiveTracking === true) {
                self.routemapService.initQueue(self.deviceId, 'device', self.locations, (realTimeLocation: any) => {
                    if ( realTimeLocation ) {
        ...
        legend.onAdd = function (map: any) {
            ...
            const labels = [self.icons.known_items,
