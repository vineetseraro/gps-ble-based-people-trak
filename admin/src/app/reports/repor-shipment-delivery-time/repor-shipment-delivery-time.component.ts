...
@Component({
  selector: 'app-repor-shipment-delivery-time',
  templateUrl: './repor-shipment-delivery-time.component.html',
  styleUrls: ['./repor-shipment-delivery-time.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReporShipmentDeliveryTimeComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'shipmentdeliverytime_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('shipment-delivery-time', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'shipmentDeliveryTime';
