...
@Component({
  selector: 'app-report-surgery-date-passed-open-cases',
  templateUrl: './report-surgery-date-passed-open-cases.component.html',
  styleUrls: ['./report-surgery-date-passed-open-cases.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportSurgeryDatePassedOpenCasesComponent implements OnInit {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('orders-not-closed', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'ordersNotClosed';
