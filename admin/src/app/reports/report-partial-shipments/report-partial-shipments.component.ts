...
@Component({
  selector: 'app-report-partial-shipments',
  templateUrl: './report-partial-shipments.component.html',
  styleUrls: ['./report-partial-shipments.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportPartialShipmentsComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'partialshipments_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('partial-shipments', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'partialShipments';
