...
@Component({
  selector: 'app-report-most-used-equipment-per-surgeon',
  templateUrl: './report-most-used-equipment-per-surgeon.component.html',
  styleUrls: ['./report-most-used-equipment-per-surgeon.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportMostUsedEquipmentPerSurgeonComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'mostusedequipmentpersurgeon_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('most-used-products-per-surgeon', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'mostUsedProductsPerSurgeon';
