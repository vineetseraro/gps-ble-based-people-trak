...
@Component({
  selector: 'app-report-sensor-connection-status',
  templateUrl: './report-sensor-connection-status.component.html',
  styleUrls: ['./report-sensor-connection-status.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportSensorConnectionStatusComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'sensorstatus_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('sensor-connection-status', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'sensorConnectionStatus';
