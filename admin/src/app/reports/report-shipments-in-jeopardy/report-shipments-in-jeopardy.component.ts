...
@Component({
  selector: 'app-report-shipments-in-jeopardy',
  templateUrl: './report-shipments-in-jeopardy.component.html',
  styleUrls: ['./report-shipments-in-jeopardy.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportShipmentsInJeopardyComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'shipmentsinjeopardy_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('shipments-in-jeopardy', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'shipmentsInJeopardy';
