...
@Component({
  selector: 'app-report-carrierwise-delayed-delivery',
  templateUrl: './report-carrierwise-delayed-delivery.component.html',
  styleUrls: ['./report-carrierwise-delayed-delivery.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportCarrierwiseDelayedDeliveryComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'shipmentdeliverytimecarrier_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('carrier-wise-delayed-shipments', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'carrierWiseDelayedShipments';
