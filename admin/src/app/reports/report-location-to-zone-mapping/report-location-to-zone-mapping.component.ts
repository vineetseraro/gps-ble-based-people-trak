...
@Component({
  selector: 'app-report-location-to-zone-mapping',
  templateUrl: './report-location-to-zone-mapping.component.html',
  styleUrls: ['./report-location-to-zone-mapping.component.css'],
  providers: [OrderService, GlobalService]
})

export class ReportLocationToZoneMappingComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'locationzones_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('location-zone-mapping', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'locationToZoneMapping';
