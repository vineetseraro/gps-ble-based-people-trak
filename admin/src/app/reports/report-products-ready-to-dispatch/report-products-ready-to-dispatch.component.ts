...
@Component({
  selector: 'app-report-products-ready-to-dispatch',
  templateUrl: './report-products-ready-to-dispatch.component.html',
  styleUrls: ['./report-products-ready-to-dispatch.component.css'],
  providers: [OrderService, GlobalService]
})
export class ReportProductsReadyToDispatchComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.searchService.notifyObservable$.subscribe((res) => {
      if (res.hasOwnProperty('option') && res.option === 'productsreadytodispatch_search') {
 ...
 onResize(event:any) {
  ...
  public getOrderList(query: string) {
    this.loader = true;
    this.orderService.getReports('products-ready-to-dispatch', query).subscribe((data:any) => {
  ...
  public export() {
    ...
    if ( this.totalRecords > 0) {
      let format = 'csv', entity = 'productsReadyToDispatch';
